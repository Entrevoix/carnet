...
## Blocking prerequisite: cleartext to loopback and LAN

`apps/mobile/android/app/src/main/AndroidManifest.xml` — the **release**
manifest — has no `usesCleartextTraffic`. It appears only in the `debug` and
`debugOptimized` variants. On targetSdk >= 28 the default is `false`, so on a
release APK:

- `http://127.0.0.1:8080` (Relais on the handset) is blocked at the socket
- any `http://` LAN Relais is blocked too

This makes the entire offline story fail on exactly the builds users install,
while working in dev builds — which is very likely why it has not surfaced (the
2026-07-23 local-LLM end-to-end verification would have been a dev build).

`apps/mobile/android/` is gitignored and regenerated by `expo prebuild`, so this
must be fixed in a **config plugin** (`plugins/withCleartextLocal.js`), joining
the five that already exist.

Scope it narrowly: ship an Android network security config permitting cleartext
**only** to loopback and RFC1918 ranges, not a blanket
`usesCleartextTraffic="true"`. Cleartext to arbitrary internet hosts stays
blocked, consistent with the SSRF work in #70.

**Unverified.** The manifest evidence is strong but the honest check is a
release APK against a local Relais. That verification is task 1 of the plan; if
it turns out cleartext already works via some path not visible in the manifest,
the plugin is dropped and nothing else in this design changes.
...
0. **Cleartext plugin + device verification.** Confirm on a release APK that
   `http://127.0.0.1:8080` is in fact blocked, add `withCleartextLocal.js`
   scoped to loopback + RFC1918, and confirm it is then reachable. Nothing else
   depends on being right about this, but everything else is pointless if it is
   wrong. **Ships independently — it fixes a live bug in today's local-LLM
   feature, regardless of whether the rest of this design proceeds.**
...
- **#85** (backend switch UX + offline capture E2E) — **its airplane-mode
  ...
  provider. It is gated on phase 0: without the cleartext plugin the test cannot
  pass on a release build. The UX half is widened by the provider list.
